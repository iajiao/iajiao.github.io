...
share: true
---
博客托管在GitHub Pages上，非常方便。不过，对于图片如何处理，又是非常的不方便。一直以来，想用R2作图床。然而，毕竟并非计算机科班出身，就是学不会，遇到各种问题。念念不忘，必有回响，这次终于会了。不仅成功设置了图床，还用R2搭建了一个瀑布流相册，多年以来的溯源呀！
...
4. 在存储桶的 **设置** 中, 找到 **自定义域名**
5. 点击 **连接域名**, 输入你想使用的子域名, 比如 `img.yourdomain.com`
...
5. 点击 **创建 API 令牌**
6. 记录下生成的 **Access Key ID** 和 **Secret Access Key**
同时记下 **Cloudflare Account ID**, 可以在 Dashboard 右侧边栏找到。
...
r2_gallery:
  public_url: "https://img.ajiao.eu.org"
```
